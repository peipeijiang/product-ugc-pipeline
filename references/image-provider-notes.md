...
| Param | Value | Notes |
|---|---|---|
| `aspect_ratio` | derived from `--size` | `auto` by default. Chronological storyboards default to `--size 1080x1920`, so the derived ratio is `9:16` and matches the vertical video contract. Identity and usage sheets pass their own grid canvas from `v2_contract.SPECS` and keep that grid proportion. Passing `--size 1024x1536` still yields `2:3`, which must not be used for a storyboard feeding a 9:16 video. Pass an explicit `--image-aspect-ratio` to override. |

...
4. **Omni/non-VEO still needs generated scene grounding.** Omni Flash can accept multiple references rather than strict first/final frames, but it must still receive at least one Image2-generated scene/person/use reference frame plus verified product references when supported. Do not submit Omni with only raw product images for UGC scenes.

...
4. **Omni Flash has two explicit image modes and defaults to 10 seconds.** `omni-flash` supports 4/6/8/10 seconds and this skill defaults it to 10 seconds. Use `--reference-mode first-last` for an exact generated start/end pair, or `--reference-mode omni-reference` for 1–3 all-purpose references. The all-purpose mode must include at least one Image2-generated scene/person/use frame and may add the verified identity sheet and canonical product photo. Do not submit Omni with only raw product images for UGC scenes. Never silently treat all-purpose references as a forced first/final pair.
   For an explicitly requested storyboard-led reroll, `reference_images` in the selected prompt variant may provide an ordered set of up to three references, typically: Image2-generated chronological storyboard, QC-passed identity grid, canonical real product photo. The storyboard must carry current Image2 provenance, pass explicit keyframe QC, and show only one physical product instance per panel. Real product photos remain authoritative if the storyboard and identity grid conflict.
5. **Record the reference chain.** Each generated video result should make it clear which local generated frame(s) and product reference(s) were used, so later review can tell whether the media followed the correct pipeline.
...
- For 10-second omni-flash clips, the spoken copy must be written to finish naturally inside 10 seconds at normal creator pace: target 18–22 English words total, hard maximum 25 words, and no more than 3 short lines. Do NOT reuse 8-second word counts for 10s videos — each duration needs its own word budget. Voiceover must end fully before video stops; if the spoken line would take more than 10 seconds at normal pace, rewrite it shorter rather than hoping the model will speed up or the video will extend.
- Omni Flash provider requests use exactly `model`, `prompt`, and `params`; `params` contains `images` (optional, maximum 3), `aspect_ratio`, `duration`, `enhance_prompt`, and optional `enable_upsample`. Use `POST /v1/media/generate`, poll `GET /v1/media/status?task_id=...`, treat `is_final` as the terminal flag and `state` as the success/failure flag. Keep Omni prompts at or below 4,000 characters.

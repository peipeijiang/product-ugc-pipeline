...
4. **Omni Flash has two explicit image modes and defaults to 10 seconds.** `omni-flash` supports 4/6/8/10 seconds and this skill defaults it to 10 seconds. Use `--reference-mode first-last` for an exact generated start/end pair. In v2, `--reference-mode omni-reference` sends an Image2 chronological storyboard and the current QC-passed product identity grid. When `usage_poses/manifest.json` records a distinct operation grid, the adapter requires usage QC and adds it as image 3. Reject more than three references before paid submission. Never treat all-purpose references as a forced first/final pair.
